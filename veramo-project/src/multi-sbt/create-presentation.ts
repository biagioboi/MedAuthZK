...
const sourcePath = path.join('.', 'zokrates', 'circuit-trattamentosingolo', 'circuit.zok');

// Funzione per generare la prova ZKP
async function generateZKP(diagnosisHashBreve: Int16Array) {
  ...
  // Calcola il witness
  const inputs = [diagnosisHashBreve];
  const { witness } = await zokratesProvider.computeWitness(artifacts.program, inputs);

  // Carica la proving key
  const proveKeyPath = path.join('.', 'zokrates', 'circuit-trattamentosingolo', 'proving.key');
