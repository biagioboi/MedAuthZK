...
async function issueSBT() {
    try {
      ...
      const presentation: Presentation = JSON.parse(
        fs.readFileSync(presentationPath, "utf-8")
      );
  
      // Validazione dei dati
      if (
        !presentation.credentialSubject ||
      ...
      const events: ethers.Event[] = await sbtContract.queryFilter(
        filter,
        receipt.blockNumber
      );
      ...
      if (events.length > 0 && events[0].args?.tokenID) {
        ...
        console.log(
          `SBT emesso con successo con Token ID: ${tokenID.toString()}`
        );
      } else {
        console.log(
          "Evento SBTIssued non trovato nella ricevuta della transazione."
        );
      }
    } catch (error) {
